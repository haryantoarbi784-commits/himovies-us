...
// Utility untuk membuat slug dari nama TV show
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Himovies - ${tvShowData.name}`,
    description: tvShowData.overview,
    openGraph: {
      title: tvShowData.name,
      description: tvShowData.overview,
      url: `https://himovies-us.netlify.app/tv-show/${slug}`,
      siteName: 'Himovies',
    ...
    <div className="min-h-screen bg-slate-900 text-white pb-8">
      {backdropUrl && (
        <div className="relative h-64 sm:h-96 md:h-[500px] overflow-hidden">
          <Image src={backdropUrl} alt={`${tvShowData.name} backdrop`} fill style={{ objectFit: 'cover' }} className="w-full h-full object-cover" priority />
      ...
      <div className="p-4 sm:p-8 md:p-12 relative -mt-32 md:-mt-48 z-10">
        <div className="flex flex-col md:flex-row items-start md:space-x-8">
          <div className="w-full md:w-1/3 flex-shrink-0 mb-6 md:mb-0">
            <Image src={posterUrl || `/placeholder-image.png`} alt={tvShowData.name} width={500} height={750} className="w-full h-auto rounded-lg shadow-xl" priority unoptimized={!posterUrl} />
          </div>
          <div className="flex-1">
            <h1 className="text-4xl sm:text-5xl font-extrabold text-blue-400 mb-2">{tvShowData.name}</h1>
            <p className="text-gray-300 text-lg sm:text-xl mb-4 italic">{tvShowData.tagline}</p>
            <div className="flex items-center space-x-4 mb-4">
              <span className="flex items-center bg-blue-600 rounded-full px-3 py-1 text-sm font-semibold text-white"><FaStar className="text-yellow-400 mr-1" />{tvShowData.vote_average.toFixed(1)} / 10</span>
              <span className="text-gray-400 text-sm">{tvShowData.first_air_date?.substring(0, 4)}</span>
              <span className="text-gray-400 text-sm">{tvShowData.number_of_seasons ? `${tvShowData.number_of_seasons} Seasons` : 'N/A'}</span>
            </div>
            <h2 className="text-2xl font-bold mt-6 mb-2">Synopsis</h2>
            <p className="text-gray-300 text-justify mb-6">{tvShowData.overview || 'Synopsis not available.'}</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-gray-400 mb-6">
              <p><strong>Genre:</strong> {tvShowData.genres?.map((genre) => genre.name).join(', ')}</p>
              <p><strong>Status:</strong> {tvShowData.status}</p>
              <p><strong>Creator:</strong> {crew.find(member => member.job === 'Creator')?.name || 'N/A'}</p>
              {tvShowData.homepage && (<p><strong>Website:</strong> <a href={tvShowData.homepage} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:underline">Visit Website</a></p>)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 sm:px-8 md:px-12">
              ...
              {cast.map((actor) => (
                <div key={actor.id} className="flex-shrink-0 w-24 text-center">
                  <div className="w-24 h-24 rounded-full overflow-hidden mb-2 border-2 border-gray-600">
                    {actor.profile_path ? <Image src={`https://image.tmdb.org/t/p/w200${actor.profile_path}`} alt={actor.name} width={96} height={96} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-700 flex items-center justify-center"><FaUserCircle className="text-4xl text-gray-400" /></div>}
                  </div>
        ...
        {trailer && (
          <div className="mt-8 border-t border-gray-700 pt-8">
            ...
            <div className="aspect-w-16 aspect-h-9"><iframe className="w-full aspect-video rounded-xl shadow-lg" src={`https://www.youtube.com/embed/${trailer.key}`} title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe></div>
          </div>
        )}

        <div className="mt-8 border-t border-gray-700 pt-8">
          <h2 className="text-2xl font-bold mb-4 text-blue-400">User Reviews</h2>
          {userReviews.length > 0 ? (<div className="space-y-4">{userReviews.map((review) => (<div key={review.id} className="bg-gray-800 p-4 rounded-lg shadow-md"><p className="font-semibold text-white">{review.author}</p><p className="text-sm text-gray-300 mt-1 text-justify line-clamp-5">{review.content}</p></div>))}</div>) : (<p className="text-gray-400">No reviews for this TV show yet.</p>)}
        </div>
        ...
        <div className="mt-12 text-center">
            <Link href={`/tv-show/${slug}/stream`}>
              <button className="bg-blue-700 hover:bg-red-700 text-white font-bold py-4 px-10 rounded-lg text-xl transition-transform transform hover:scale-105 shadow-lg">🎬 Stream Now</button>
            </Link>
        </div>
      </div>
    </div>
